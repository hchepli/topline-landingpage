import { SectionTitle } from "../ui/layout/SectionTitle"
import { BrandCardGrid } from "../ui/layout/BrandCardGrid"

export function BrandGroupSection() {
  return (
    <section className="grupo min-h-screen w-full flex items-center justify-center pt-8 md:pt-10">
      <div className="w-full max-w-6xl flex flex-col items-center text-center gap-6 md:gap-8">
        <SectionTitle
          title="Grupo TF"
          description="Com mais de 16 anos de história, o Grupo TF integra empresas líderes em soluções para a construção civil. Atuando por meio das marcas TopFusion, TopFlux e TopFiber, o grupo combina engenharia de projetos, gestão comercial e inovação para atender grandes clientes e expandir sua presença em todo o território nacional."
        />

        <BrandCardGrid />
      </div>
    </section>
  )
}
